...
int main(int argc, char *argv[])
{
    ...
    if (argc < 3)
    {
        fprintf(stderr, "Usage: %s <PEM RSA Public Key File> <PEM RSA Private Key File>\n", argv[0]);
        exit(1);
    }
    ...
    encrypt = fopen("encrypted", "wb+");
    if (!encrypt)
    {
        perror("encrypted");
        fprintf(stderr, "Error loading out File.\n");
        exit(2);
    }


    rv = do_evp_seal(rsa_pkey_file, stdin, encrypt);
    fseek(encrypt, 0, SEEK_SET);
    rv = do_evp_unseal(rsa_skey_file, encrypt, stdout);

    fclose(rsa_pkey_file);
    fclose(rsa_skey_file);
    fclose(encrypt);
    return rv;
}
